...
const LevelsevenPage: React.FC = () => {
  ...
  const submitScores = useCallback(async () => {
    try {
      const response = await axios.post(
        `https://pt-9ffdb6ad-c541-4d3d-88f7.cranecloud.io/api/v1/progress`,
        {
          ...
          levelId: "67c86a043eb5a416e5d17828",
          score: 10,
          completed: true,
        }
      );
    ...
    router.push("/challenges/movie/level-eight");
  }, [email, router]);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data.action === "completed") {
        submitScores();
        toast.success("Level 7 completed!");
      } else {
        toast.error("Level 7 failed!. Please try again");
      }
    };
  ...
  return (
    <>
      ...
      >
        <iframe
          src="https://games-5qkr.onrender.com/movie?lang=en&level=7"
          style={{ width: "95%", height: "95%" }}
          frameBorder="0"
          allowFullScreen
          title="Level seven Game"
        ></iframe>
      </div>
    </>
  );
