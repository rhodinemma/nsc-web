...
const LevelFivePage: React.FC = () => {
  ...
  const submitScores = useCallback(async () => {
    try {
      const response = await axios.post(
        `https://pt-9ffdb6ad-c541-4d3d-88f7.cranecloud.io/api/v1/progress`,
        {
          ...
          levelId: "67ab6a158663464f4256686b",
          score: 10,
          completed: true,
        }
      );
    ...
    router.push("/challenges/turtle/level-six");
  }, [email, router]);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data.action === "completed") {
        ...
        toast.error("Level 5 failed!. Please try again");
      }
    };
  ...
  return (
    <>
      ...
      >
        <iframe
          src="https://games-5qkr.onrender.com/turtle?lang=en&level=5"
          style={{ width: "95%", height: "95%" }}
          frameBorder="0"
          allowFullScreen
          title="Level Five Game"
        ></iframe>
      </div>
    </>
  );
