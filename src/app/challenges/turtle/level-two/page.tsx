...
const LevelTwoPage: React.FC = () => {
  ...
  const submitScores = useCallback(async () => {
    try {
      const response = await axios.post(
        `https://pt-9ffdb6ad-c541-4d3d-88f7.cranecloud.io/api/v1/progress`,
        {
          ...
          levelId: "67ab6a098663464f42566865",
          score: 10,
          completed: true,
        }
      );
    ...
    router.push("/challenges/turtle/level-three");
  }, [email, router]);
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data === "completed") {
        ...
        toast.error("Level 2 failed!. Please try again");
      }
    };
  ...
  return (
    <>
      ...
      >
        <iframe
          src="https://games-5qkr.onrender.com/turtle?lang=en&level=2"
          style={{ width: "95%", height: "95%" }}
          frameBorder="0"
          allowFullScreen
          title="Level Two Game"
        ></iframe>
      </div>
    </>
  );
